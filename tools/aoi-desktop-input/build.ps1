<#
.SYNOPSIS
  Build the Aoi desktop-input helper (aoi_desktop_input.exe).

.DESCRIPTION
  Compiles aoi_desktop_input.cpp with MSVC (cl.exe). If cl.exe is already on
  ...
  Output: aoi_desktop_input.exe next to the source.
...
$src = Join-Path $PSScriptRoot 'aoi_desktop_input.cpp'
$out = Join-Path $PSScriptRoot 'aoi_desktop_input.exe'
...
# cl flags. ole32/oleaut32/user32/psapi are pulled in via #pragma comment(lib,...).
...
$objFile = Join-Path $PSScriptRoot 'aoi_desktop_input.obj'
