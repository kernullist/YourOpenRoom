...
4. Planner / Context Scout 계약 생성
5. Primary Worker, 조건부 Alternative Worker, Reviewer, Integrator 단계 실행
6. 검증 재실행
...
Kira 는 기본적으로 1개의 Primary Worker 를 사용합니다. 작업이 고위험이거나 모호하거나 runtime 검증이
중요하거나 deep mode 일 때만 Alternative Worker 1명을 격리된 patch challenger 로 켤 수 있습니다.
Alternative Worker 는 별도 git worktree 에서 Primary 와 다른 접근의 attempt 를 만들고, Reviewer 와
...
- 상대 워크스페이스 경로 기반의 빈 파일/폴더 생성과 중복 생성 방지
- 텍스트 검색
- 심볼 검색
- 참조 조회
- export 목록
