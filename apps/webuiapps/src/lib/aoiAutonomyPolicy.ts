...
>[] = [
  ...
  {
    // HP1: real-PC running-process inspection (metadata-only: image name +
    // pid + coarse counts, never command-line arguments). Machine-scoped,
    // default OFF, additionally gated by the host-bridge kill switch.
    id: 'process-activity',
    kind: 'process_activity',
    label: 'Running process list (metadata only)',
    enabled: false,
    scope: 'explicit_target',
    risk: 'high',
    allowedOperations: ['read_metadata', 'summarize_counts'],
