...
const AOI_CAPABILITY_REGISTRY = {
  ...
  ghidra_find_binary: {
    name: 'ghidra_find_binary',
    label: 'Ghidra Lab binary find',
    kind: 'tool',
    surface: 'automation',
    risk: 'low',
    description:
      'List or search filenames inside the folders registered as Ghidra Lab binary roots (names and sizes only; no file content).',
    access: ['read'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes:
      'Requires the os_ghidra_analysis capability. Reach is limited to the registered binary roots, re-checked after symlink resolution.',
  },
  ghidra_session_list: {
    name: 'ghidra_session_list',
    label: 'Ghidra Lab session list',
    kind: 'tool',
    surface: 'automation',
    risk: 'low',
    description:
      'List open Ghidra analysis sessions plus the lab preflight verdict (Ghidra version, JDK version, which modes are usable and why).',
    access: ['read'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes: 'Requires the os_ghidra_analysis capability.',
  },
  ghidra_analyze_start: {
    name: 'ghidra_analyze_start',
    label: 'Ghidra Lab analysis start',
    kind: 'tool',
    surface: 'automation',
    risk: 'high',
    description:
      'Propose starting headless Ghidra (pyghidra-mcp) on a binary; records a pending operator approval and starts nothing by itself.',
    access: ['execute'],
    sandboxEligible: false,
    approval: 'user-confirmation',
    promptVisible: true,
    policyNotes:
      'Requires os_ghidra_analysis + the binary inside a registered root + a passing preflight (a wrong JDK is refused here because Ghidra would prompt on stdin and hang). Starts a real process only after an Approve & Run click.',
  },
  ghidra_query: {
    name: 'ghidra_query',
    label: 'Ghidra Lab query',
    kind: 'tool',
    surface: 'automation',
    risk: 'medium',
    description:
      'Ask one bounded read question of an open Ghidra session (imports, exports, strings, functions, xrefs, decompile, call graph, semantic search).',
    access: ['read'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes:
      'Requires os_ghidra_analysis. Sub-commands are an allowlist mapped to single MCP tools; unknown sub-commands, database mutators and raw byte reads are refused before reaching the engine.',
  },
  ghidra_report_run: {
    name: 'ghidra_report_run',
    label: 'Ghidra Lab sweep and report',
    kind: 'tool',
    surface: 'automation',
    risk: 'high',
    description:
      'Propose a full ten-stage sweep of an open session ending in a written, evidence-cited report; records a pending operator approval and starts nothing by itself.',
    access: ['read', 'execute'],
    sandboxEligible: false,
    approval: 'user-confirmation',
    promptVisible: true,
    policyNotes:
      'Requires os_ghidra_analysis and a ready session. Runs for many minutes and may spawn capa as a second process. Every claim in the resulting report must cite a collected evidence anchor; uncited claims are deleted and the count is printed in the report.',
  },
  ghidra_report_read: {
    name: 'ghidra_report_read',
    label: 'Ghidra Lab report read',
    kind: 'tool',
    surface: 'automation',
    risk: 'low',
    description: 'List sweep runs and their per-stage progress, or read one finished report.',
    access: ['read'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes: 'Requires os_ghidra_analysis. Report text is truncated to a fixed budget.',
  },
  ghidra_session_stop: {
    name: 'ghidra_session_stop',
    label: 'Ghidra Lab session stop',
    kind: 'tool',
    surface: 'automation',
    risk: 'low',
    description:
      'Close a Ghidra session this lab started, freeing its JVM and port. The Ghidra project is kept, so a later session reuses the analysis.',
    access: ['execute'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes: 'Requires os_ghidra_analysis. Only sessions this lab spawned can be stopped.',
  },
  ida_gui_attach: {
    name: 'ida_gui_attach',
  ...
  // The rest of the desktop-input surface. Graded by what they can do, not by
  // how they are delivered: desktop_key can save, submit or delete through a
  // shortcut, so it is not a lesser thing than a click.
  desktop_apps: {
    name: 'desktop_apps',
    label: 'Desktop: list apps',
    kind: 'tool',
    surface: 'automation',
    risk: 'medium',
    description: 'List running desktop apps that have windows, grouped by program.',
    access: ['read'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes: 'Requires the Host Bridge os_desktop_input capability. Read-only.',
  },
  desktop_click: {
    name: 'desktop_click',
    label: 'Desktop: click a control',
    kind: 'tool',
    surface: 'automation',
    risk: 'high',
    description:
      'Click a control with a specific button, count or held modifiers (right-click, double-click, ctrl+click).',
    access: ['read', 'write'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes:
      'Requires os_desktop_input. Delivered without stealing focus where possible; held modifiers need the separate synthetic-input toggle and are refused rather than dropped. Right-click opens context menus, which can reach destructive commands.',
  },
  desktop_key: {
    name: 'desktop_key',
    label: 'Desktop: send a key combo',
    kind: 'tool',
    surface: 'automation',
    risk: 'high',
    description: 'Send a keystroke or combo (ctrl+s, tab, escape, f5) to a window.',
    access: ['read', 'write'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes:
      'Requires os_desktop_input. A shortcut can save, submit, close or delete without any visible control being involved, so this is graded with the acting tools. Modifier combos need the synthetic-input toggle. Nothing can verify a keystroke landed.',
  },
  desktop_type: {
    name: 'desktop_type',
    ...
    surface: 'automation',
    risk: 'high',
    description: 'Type text into whatever holds keyboard focus in a window.',
    access: ['read', 'write'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes:
      'Requires os_desktop_input. Unaddressed by design: the text goes wherever focus is, at the caret, and cannot be read back. desktop_act with a value is preferred and provable. Credentials must never be sent this way.',
  },
  desktop_scroll: {
    name: 'desktop_scroll',
    ...
    surface: 'automation',
    risk: 'medium',
    description: 'Scroll a control, verified by reading the scroll position back.',
    access: ['read', 'write'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
    policyNotes:
      'Requires os_desktop_input. Graded lower than the other acting tools because moving a view is reversible and commits nothing -- and it is the one input action that can prove it happened.',
  },
  desktop_drag: {
    name: 'desktop_drag',
    label: 'Desktop: drag',
    ...
    surface: 'automation',
    risk: 'high',
    description: 'Drag from one control to another inside a window.',
    access: ['read', 'write'],
    sandboxEligible: false,
    approval: 'policy-gated',
    promptVisible: true,
  ...
  desktop_focus: {
    ...
    promptVisible: true,
    policyNotes:
      'Requires os_desktop_input AND the synthetic-input toggle. Changes what the operator is looking at and persists after the call, unlike the momentary focus other actions take. Not needed to make other actions work.',
  },
  browser_drive_task: {
